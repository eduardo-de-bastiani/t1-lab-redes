...
class Client:
        ...
        def collector():
            while not self.stop_tcp_info_collector.is_set():
                try:
                    ...
                    tcp_info_raw = self.sock.getsockopt(
                        socket.IPPROTO_TCP, socket.TCP_INFO, 104
                    )

                    rtt = struct.unpack_from("=I", tcp_info_raw, 40)[0] / 1000
    ...
    def _request_put(self, filename: str):
        ...
        self._send(
            Protocol.pack_message(
                Opcode.PUT, os.path.basename(filename).encode("utf-8")
            )
        )
        try:
            opcode, payload = next(self._recv_stream())
        ...
        self._start_tcp_info_collector()
        ...
        try:
            ...
            if opcode == Opcode.SUCCESS and payload == b"UPLOAD_COMPLETE":
                ...
                logger.error(
                    f"Falha na confirmação final do servidor: {payload.decode('utf-8')}"
                )

        except (OSError, StopIteration) as e:
            logger.error(f"Erro durante a transferência do arquivo: {e}")
        finally:
            self._stop_tcp_info_collector()

    def _log_transfer_metrics(
        self, file_size_bytes: int, start_time: float, end_time: float
    ):
    ...
    def _create_session_log(self):
        ...
        avg_rate = (
            (self.bytes_sent + self.bytes_received) / duration if duration > 0 else 0
        )

        with open(log_filename, "w") as f:
            f.write("--- Log da Sessão ---\n")
            f.write(
                f"Timestamp Início: {datetime.fromtimestamp(time.time() - duration).isoformat()}\n"
            )
            ...
            if self.tcp_info_log:
                ...
                for entry in self.tcp_info_log:
                    ...
                    f.write(
                        f"{ts}, {entry['rtt_ms']:.3f}, {entry['cwnd']}, {entry['retransmissions']}\n"
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Cliente interativo para serviço de arquivos."
    )
